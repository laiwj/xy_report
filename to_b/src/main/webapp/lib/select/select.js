...
define(["validator"], function( xy_validator ){
	...
	SelectControl.prototype = {
		init : function(){
			...
			this.checkValue = [];
			this.allValue = [];
			this.containerType=this.params.containerType||null;
		...
		bindEvent : function(){
			...
			if(this.containerType=='button'){
				this.container[0].addEventListener('click',function(){
					$(this).find('.select-text').addClass('change');
				})
			}
			...
			liNode.on('click',function(e){
				...
				if(value && _self.params.isCheckBox){//多选框
                    ...
                    if(_self.params.containerType=='button'){
                        _self.container.find('.select-text').removeClass('change')
                    }
					return false;
				}else{
