...
public class MUserDao extends MBaseDao<User> {
    ...
    public List<User> user_list(String user_id, int page, int pageSize) {
        ...
        return user_list(query, page, pageSize);
    }


    public List<User> user_list(Set<String> user_ids, int page, int pageSize) {
        Document query = new Document();
        if (user_ids.size() > 0)
    ...
    public List<User> user_list_by_parent_id(String pm_user_id, int page, int pageSize){
        Document query = new Document();
        if (CommonUtil.isNotEmpty(pm_user_id))
            query.append("pm_user_id", pm_user_id);
        return user_list(query, page, pageSize);
    }

    public List<User> user_list(Document query, int page, int pageSize) {
        FindIterable<Document> createTime = getCollection(collectionName)
                .find(query)
                .sort(new Document("pm_user_id", 1).append("createTime", 1));
        if (page > 0) {
...
//            user.setPower_list(null);
    ...
    public long getCount(Set<String> user_ids) {
        Document query = new Document();
        if (user_ids.size() > 0)
            query.append("pm_user_id", new Document("$in", user_ids));
    ...
    public long getCount(Document query) {
        try {
