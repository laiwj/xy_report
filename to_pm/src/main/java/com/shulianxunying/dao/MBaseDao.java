...
public class MBaseDao<TEntity> {
    ...
    public List<TEntity> findParams_by_key_values(String collectionName, Class<TEntity> cls, JSONObject key_values) {
        MongoCursor<Document> document = findParams_by_key_values(collectionName, key_values);
        List<TEntity> out = new ArrayList<>();
        while (document.hasNext()) {
            TEntity reportInfo = document_2_class(document.next(),cls);
...
//        return document_2_class(document, cls);
    ...
    public MongoCursor<Document> findParams_by_key_values(String collectionName, JSONObject key_values) {
...
//        }
        try {
            return getDatabase().getCollection(collectionName).find(query).iterator();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
