...
define([
	"util",
	"validator"
],function(xy_util,xy_validator){
	//var xy_validator = require("./");
	var xy_beanUtil =  {
	...
	{
		if ( !xy_validator.isNull(attr.value) )
		{
		...
		//非空值
		if ( xy_validator.notNullOrEmpty(value) )
		{
		...
		//设置replaceVal
		if ( xy_validator.notNullOrEmpty(attr.replaceVal) )
		{
		...
		//按类型设置值
		if ( xy_validator.nullOrEmpty(attr.valType) )
		{
			attr.valType = "string";
		}
		switch (attr.valType)
	...
	{
		if ( xy_validator.nullOrEmpty(attr.name) )
		{
			return;
		}
		...
		switch (attr.valType)
		{
			//时间格式
			case "date":
				if ( xy_validator.isEmpty(value) || value == 0 )
				{
					value = "";
				}
				else
				{
					if ( xy_validator.nullOrEmpty(attr.format) )
					{
			...
			//数据字典格式
			case "dict":
				if ( xy_validator.isEmpty(value) )
				{
					value = "";
				}
				else
				{
					if (!xy_validator.nullOrEmpty(attr.dictType) )
					{
						value = xy_dic.systemDicMap[attr.dictType][value];
					}
		...
		switch ( type.toString() )
		{
			...
			case "3":
				...
				$("#" + attr.domName).attr("title", xy_util.htmlTitleEncode(value));
				break;
			...
			case "dict":
				if ( xy_validator.isEmpty(value) )
				{
					...
					if(attr.dictKey == "educationType" && value == 0){
						...
						value = xy_dic.systemDicMap[attr.dictKey][value];
					}
