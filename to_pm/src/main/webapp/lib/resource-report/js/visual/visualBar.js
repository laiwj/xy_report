...
//    var minV = data.reduce(function(a, b) { return a > b.value ? b.value : a; }, data[0].value);
//    var dt = data.map(function(d) { return (d.value-0).toFixed(6); });
//
//    var option = {
//        tooltip: { trigger: 'item', textStyle:{ fontFamily:"楷体" },
//            formatter:function(p){ return p.name + "<br>" + title + "：" + p.data} },
//        grid:{left:100},
//        xAxis: { type: 'value',min:Math.floor(minV*0.95),  boundaryGap: [0, 0.01], axisLine:{ lineStyle:{ color:'#d3d6db'}},
...
function getOptionBar(type, data) {
    ...
    //    gdc = new echarts.graphic.LinearGradient( 0, 0, 1, 0, colorGradient );
    var color = { 'need': ["#C3EDE9", GDC(colorCfg.need)], 'index': ["#BBD1E8", GDC(colorCfg.index)] }[type];
    var minV = data.reduce(function(a, b) {
        return a >parseFloat(b.value) ? parseFloat(b.value) : a;
    }, data[0].value);
    //var dt = data.map(function(d) { return parseFloat(d.value).toFixed(6); });
    var dt = data.map(function(d) {
        return parseInt(d.value) === Number(d.value) ? d.value : parseFloat(d.value).toFixed(6);
    });
    var option = {
        tooltip: {
            trigger: 'item',
