...
define(["validator"], function( xy_validator ){
	...
	SelectControl.prototype = {
		...
		drawing : function(){
			...
			if(isCheckBox){
				arr.push('<li><cite type="checkbox" class="J_AllCheckBox check-box gridCheckBox"></cite>全选</li>');
			...
			if(itemLength == 0 && !isCheckBox){
				...
				for(var key in this.dataMaps){
					...
					if(isCheckBox){
						arr.push('<cite type="checkbox" class="check-box gridCheckBox"></cite>');
					}
		...
		bindEvent : function(){
            ...
            $_selfoptions.on("mouseleave",function(){
                $(".options-warp").hide();
            });
			...
			liNode.on('click',function(e){
				...
				if(value && _self.params.isCheckBox){//多选框
					...
					if(checked){
						$(this).find('.J_AllCheckBox').removeClass('checked');
					}else{
						$(this).find('.J_AllCheckBox').addClass('checked');
					}
		...
		clickSelectAllCheckBox: function(){
			...
			if(checkAllNode.hasClass('checked')){//多选的全部
				liNode.find('.check-box').addClass('checked');
				this.checkValue = [];
				var _self = this;
				...
				liNode.find('.check-box').removeClass('checked');
				this.checkValue = [];
			}
		...
		checkSelectedAll: function(){
			...
			if(this.params.isCheckBox){
				...
				if(count && count == this.container.find('li').size() - 1){
					checkAllNode.addClass('checked');
				}else{
					checkAllNode.removeClass('checked');
				}
			}
			checkAllNode = null;
		},
		...
		setListData: function(data , isCallChangeEvent){
			isCallChangeEvent = isCallChangeEvent !== false;
